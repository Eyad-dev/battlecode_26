...
class Player:
    ...
    def run(self, ct: Controller) -> None:
        ...
        if etype == EntityType.CORE:
            ...
            rangesquare = GameConstants.CORE_VISION_RADIUS_SQ
            rangee= (int)(rangesquare**0.5)
            for x in range(-rangee, rangee + 1):
                for y in range(-rangee, rangee + 1):
                    if x**2 + y**2 <= rangesquare:
                        ...
                        if ct.get_tile_env(pos) == Environment.ORE_TITANIUM or ct.get_tile_env(pos) ==Environment.ORE_AXIONITE:
                            print("YEAYAYF_EYFE(FEWIOFHEWOF)")
                            print("(" + str(newposx) + "," + str(newposy) + ")")
                            #FIRST IMPLEMENTATION FROM MATIKANETANNHAUSER
