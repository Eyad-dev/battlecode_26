...
def handle_vision_and_harvesting(self, ct: Controller, current_pos: Position) -> bool:
    ...
    if self.mode == "ROOMBA":
        ...
        for ore_pos in ores:
            if (ct.get_tile_env(ore_pos) == Environment.ORE_AXIONITE and self.axionite_foundary_states == 0 and self.bot_state == "AXIONITER"):
        ...
        eligible_ores = [
        o for o in ores
        if not (ct.get_tile_env(o) == Environment.ORE_AXIONITE and self.axionite_foundary_states != 0 and self.bot_state == "AXIONITER")
        ]
        if eligible_ores:
...
def run_greedy_mode(self, ct: Controller, current_pos: Position, goal_pos: Position) -> bool:
    ...
    if goal_pos == self.ourcoord:
        ...
        if (next_pos in self.core_tiles and self.axionite_foundary_states == 1 and self.bot_state == "AXIONITER"):
